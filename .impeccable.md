...
### Aesthetic Direction
Pure black (matching the physical notch), single blue accent (`Color(red: 0.30, green: 0.45, blue: 0.82)`), SF Pro system font throughout. No decorative elements. Dark mode only — there is no light mode. References: Apple's own notch indicator dots, Control Center, and the original NotchNook app. Anti-reference: anything with color diversity, heavy gradients, or typographic personality.
...
2. **SF Pro is the brand** — No custom fonts. SF Pro at small sizes on Retina is world-class. Lean into its weight range (Light through Semibold) as the primary expressive tool.
...
4. **Monospaced for numbers** — All time values, timestamps, and numeric data use `.design(.monospaced)`. Proportional numerals jump — monospaced numerals are calm.
5. **Nothing competes with content** — Tab icons, section headers, state labels, timestamps should recede. The user's data (song title, next meeting) is the foreground. Everything else is scaffolding.
